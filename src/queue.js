...
class Queue {
	...
	enqueue(element) {
		...
		if (this.head) {
			...
			this.tail.next = list; // inserts the created node after the tail of our Queue
			this.tail = list; // now the created node is the last node
		} else {
			// if there are no nodes in the Queue
			this.head = list; // the created node is a head
			this.tail = list; // also the created node is a tail in Queue because it is single.
		}
